...
$sql = "SELECT id, original_name FROM perfumes";
...
<div id="chatBox" class="chat-box">
    ...
    <select id="userMessage" class="form-control">
        <option value="" disabled selected>Select a perfume...</option>
    </select>
...
<script>
    $(document).ready(function () {
        // Load perfumes into the dropdown
        $.get('get_perfumes.php', function (data) {
            const perfumes = JSON.parse(data);
            perfumes.forEach(perfume => {
                $('#userMessage').append(`<option value="${perfume.name}">${perfume.name}</option>`);
            });
        });
