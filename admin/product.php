...
$select = "
    SELECT p.*, 
           ...
           GROUP_CONCAT(CONCAT(pf.format) ORDER BY pf.id ASC SEPARATOR ', ') as formats,
           GROUP_CONCAT(pf.price ORDER BY pf.id ASC SEPARATOR ', ') as prices,
           GROUP_CONCAT(pf.unit_of_measure ORDER BY pf.id ASC SEPARATOR ', ') as unit_of_measure,
                     ...
                     <tbody>
                        <?php while ($row = mysqli_fetch_array($res)) { ?>
                           <tr>
                              ...
                              <td> <?php
                                    $formats = explode(", ", $row['formats']); // Split formats into an array
                                    $units = explode(", ", $row['unit_of_measure']); // Split units into an array

                                    $output = [];
                                    foreach ($formats as $index => $format) {
                                       // Combine each format with its corresponding unit of measure
                                       $output[] = $format . " (" . ($units[$index] ?? "") . ")";
                                    }

                                    echo implode("<br>", $output); // Join the results with <br> for line breaks
                                    ?> </td>
